...
async function findOrCreateUser(telegramUser) {
  try {
    ...
    if (!users[userId]) {
      ...
      // Migration: Add isTeamHandler field for existing users
      if (users[userId].isTeamHandler === undefined) {
        users[userId].isTeamHandler = false; // Default to false for existing users
        await writeJsonFile(USERS_FILE, users);
      }
    }
...
bot.onText(/\/status/, async (msg) => {
  ...
  if (todayRecord) {
    ...
    const normalPoints = getTarbiyahPoints(user.isTeamHandler || false, false);
...
bot.onText(/\/settim/, async (msg) => {
  ...
  const currentStatus = user.isTeamHandler ? 'Handle Tim' : 'Tidak Handle Tim';
